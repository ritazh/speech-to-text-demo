$(function () {
    ...
    var p1 = new Primer("#primer", 400, 400);
    ...
    window.moveLastToCell = function(newPosition)
    {
    ...
    window.updateGrid = function() {
        ...
        for ( var i = 0; i < totalCells; i++ ) {
            var cell = new Primer.Layer();
            var _x = i%5;
            var _y = Math.floor(i/5);
            var xPos = _x*(cellSize+cellGutter);
            var yPos = _y*(cellSize+cellGutter);
            cell.fillStyle = cellData[i]; //"#aeaeae";
            cell.fillRect(xPos, yPos, cellSize, cellSize);
            cell.stroke();
            p1.addChild(cell);

            var label = new Primer.Layer();
            //var id = String.fromCharCode(64+index);
            label.fillStyle = "#636363";
            label.textAlign = "center";
            label.fillText(i+1, xPos, yPos + (.35*cellSize), cellSize );
            p1.addChild(label);
        }
    }
